...
def viewpoint(img_filenames, class_idx, output_result_files):
    ...
    for i in range(len(img_filenames)):
        ...
        fout = open(output_result_files[i], 'w')
        fout.write('%d %d %d\n' % (
        preds[0][i] % 360, preds[1][i] % 360, preds[2][i] % 360))
        fout.close()
...
def get_top_preds(prob, num_bin=8, diff_threshold=10):
    ...
    for i in range(0, num_bin):
        ...
        for i, ituple in enumerate(preds_list):
            ipred, iprob = ituple
            if min(abs(local_pred - ipred),
                   N - abs(local_pred - ipred)) < diff_threshold:
                if local_prob > iprob:
                    preds_list[i] = (
                    local_pred, local_prob)  # exchange to bigger one
...
def get_topk_viewpoints(probs_3dview, topk):
    ...
    for i in range(min(topk, len(preds_list_azimuth))):
        apred, aprob = preds_list_azimuth[i]
        viewpoints_list.append(((apred, probs_3dview[1].argmax(),
                                 probs_3dview[2].argmax()), aprob))

    viewpoints_list = sorted(viewpoints_list, key=lambda item: item[1],
                             reverse=True)
...
# @mhsung
def viewpoint_pred(img_filenames, class_idx, output_result_files):
    batch_size = g_test_batch_size
    model_params_file = g_caffe_param_file
    model_deploy_file = g_caffe_deploy_file
    result_keys = g_caffe_prob_keys
...
def test_avp_nv(cls_names, img_name_file_list, det_bbox_mat_file_list,
                result_folder):
    C = len(cls_names)
    ...
    for i, class_idx in enumerate(class_idxs):
        img_filenames = [x.rstrip().split(' ')[0] for x in
                         open(img_name_file_list[i])]
        ...
        output_result_file = os.path.join(result_folder,
                                          cls_names[i] + '_pred_view.txt')
        ...
        BASE_DIR, cls_names[i], det_bbox_mat_file_list[i], output_result_file,
        result_folder, 0)
        print matlab_cmd
        os.system('%s -nodisplay -r "try %s ; catch; end; quit;"' % (
        g_matlab_executable_path, matlab_cmd))
...
def test_vp_acc(cls_names, img_name_file_list, result_folder,
                view_label_folder):
    ...
    for i, class_idx in enumerate(class_idxs):
        tmp_img_filenames = [x.rstrip().split(' ')[0] for x in
                             open(img_name_file_list[i])]
    ...
    matlab_cmd = "addpath('%s'); test_gt('%s','%s');" % (
    BASE_DIR, result_folder, view_label_folder)
    print matlab_cmd
    os.system('%s -nodisplay -r "try %s ; catch; end; quit;"' % (
    g_matlab_executable_path, matlab_cmd))
